...
exports.createVenta = async (req, res) => {
  ...
  if (!sale_datetime) {
    return res.status(400).render('cantera/vender', { clients, products, error: 'Falta fecha y hora' });
  }
  ...
  if (manual) {
    ...
    if (!name) {
      return res.status(400).render('cantera/vender', { clients, products, error: 'Cliente manual: falta nombre' });
    }
    const created = await store.createClient({ full_name: name, phone: phone || '' });
    finalClientId = created.id;
  } else {
    if (!client_id) {
      return res.status(400).render('cantera/vender', { clients, products, error: 'Debe seleccionar un cliente' });
    }
  ...
  if (!product_id) {
    return res.status(400).render('cantera/vender', { clients, products, error: 'Debe seleccionar un producto' });
  }
  ...
  return res.redirect('/cantera/semanal');
};
...
exports.viewSemanalSelector = async (req, res) => {
  ...
  if (weekStart) {
    ...
    let sales = await store.listSalesByTypeBetween('CANTERA', weekStart, weekEnd);
    ...
    // lista (opcional, para ver “qué compone” la semana)
...
exports.viewResumenDia = async (req, res) => {
  ...
  if (!dayISO) return res.redirect('/cantera/semanal');

  const clients = await store.listClients();
  const salesRaw = await store.listSalesByTypeOnDay('CANTERA', dayISO, clientId || null);
  ...
  res.render('cantera/resumen_dia', {
    date: dayISO,
    clients,
    clientId,
    sales,
...
exports.registrarPago = async (req, res) => {
  const { sale_id, mode, amount, pay_method, redirect_date, redirect_clientId, redirect_weekStart, redirect_month, redirect_year } = req.body;

  const sale = await store.getSale(sale_id);
  if (!sale) return res.redirect('/cantera/semanal');
  ...
  if (deuda <= 0) {
    return res.redirect(`/cantera/resumen/dia?date=${redirect_date}&clientId=${redirect_clientId || ''}&${backQs}`);
  }
